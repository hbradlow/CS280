...
labels = load(Cfg.LABEL_FILE);

BOX_JITTER_PIXELS = [5 10];

%% Extract positive and negative patches from user-labeled regions
bar = waitbar(0, 'Extracting patches');
...
for i=1:length(labels.frames)
  ...
  waitbar(i/length(labels.frames), bar);
  
  % For each labeled box, output a positive patch there
  p = imcrop(frame, box);
  imwrite(p, sprintf('%s/img_%d.png', Cfg.POS_EXAMPLES_DIR, i));
  % Move the box around for more positive patches
  k = 0;
  for j=BOX_JITTER_PIXELS
    newbox = [box(1)+j, box(2)+j, box(3), box(4)];
    imwrite(imcrop(frame, newbox), sprintf('%s/img_%d_%d.png', Cfg.POS_EXAMPLES_DIR, i, k));
    k = k + 1;
    newbox = [box(1)+j, box(2)-j, box(3), box(4)];
    imwrite(imcrop(frame, newbox), sprintf('%s/img_%d_%d.png', Cfg.POS_EXAMPLES_DIR, i, k));
    k = k + 1;
